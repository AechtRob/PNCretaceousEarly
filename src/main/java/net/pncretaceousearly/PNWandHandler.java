...
import net.pncretaceousearly.world.dimension.cretaceousearly.WorldCretaceousEarly;
    ...
    public void useWand(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (levelEnchantment > 0) {
            ...
            if (world.isAirBlock(pos1)) {
                ...
                boolean portalSpawnCretaceousEarly = WorldCretaceousEarly.portal.portalSpawn(world, pos1);

                if (portalSpawnCretaceousEarly) {
    ...
    public void goToNether(EntityTravelToDimensionEvent event) {
        if (LepidodendronConfig.oneWayPortalsNether && LepidodendronConfig.oneWayPortals) {
            if (event.getDimension() == -1) {
                ...
                if (event.getEntity().getEntityWorld().provider.getDimensionType().getId() == LepidodendronConfig.dimCretaceousEarly) {
                    if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
                        ...
                        if (thePlayer.dimension != event.getDimension()) {
                            ...
                            WorldCretaceousEarly.BlockCustomPortal.transferPlayerToDimensionPN(thePlayer.server.getPlayerList(), thePlayer, event.getDimension(), WorldCretaceousEarly.BlockCustomPortal.getTeleporterForDimension(thePlayer, entityIn.getPosition(), event.getDimension()));
                        }
                    }
                    event.setCanceled(true);
                }
