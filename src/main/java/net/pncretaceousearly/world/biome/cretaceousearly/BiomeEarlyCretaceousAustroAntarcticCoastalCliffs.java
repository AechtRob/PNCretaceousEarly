...
public class BiomeEarlyCretaceousAustroAntarcticCoastalCliffs extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:cretaceous_early_austro_antarctic_coastal_cliffs")
	public static final BiomeGenCustom biome = null;
	public BiomeEarlyCretaceousAustroAntarcticCoastalCliffs(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.ICE)) {
				{
					...
					for (int j = 0; j < i; ++j) {
						...
						NEST_GENERATOR.generate(worldIn, rand, blockpos, 102, new EntityPrehistoricFloraMythunga(worldIn));
					}
...
//						BlockPos blockpos = worldIn.getHeight(pos.add(k, 0, l));
//						blockpos = ChunkGenSpawner.getTopSolidBlock(blockpos, worldIn).up();
//						NEST_GENERATOR.generate(worldIn, rand, blockpos, 102, new EntityPrehistoricFloraThapunngaka(worldIn));
//					}
					...
					for (int j = 0; j < i; ++j)
					{
						...
						BlockPos blockpos = worldIn.getHeight(pos.add(k, 0, l));
						blockpos = ChunkGenSpawner.getTopSolidBlock(blockpos, worldIn).up();
						NEST_GENERATOR.generate(worldIn, rand, blockpos, 102, new EntityPrehistoricFloraHaliskia(worldIn));
					}
				}
			}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 4; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					LEAFBLOCK_GENERATOR.generate((BlockBush) BlockHirmeriellaSapling.block, BlockHirmeriellaLeaves.block.getDefaultState(), BlockHirmeriellaLog.block.getDefaultState().withProperty(BlockHirmeriellaLog.BlockCustom.FACING, EnumFacing.NORTH), worldIn, rand, pos.add(j, l, k), 0, 255);
				}
