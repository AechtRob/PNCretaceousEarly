...
public class BiomeEarlyCretaceousLandAsiaYixianLakesA extends ElementsLepidodendronMod.ModElement {
	...
	static class BiomeGenCustom extends BiomeCretaceousEarly {
		public BiomeGenCustom() {
			super(new BiomeProperties("E. Cretaceous Asia Coniferous Lakelands").setBaseHeight(4.9F).setHeightVariation(0.29F).setTemperature(0.8F));
		...
		public void decorate(World worldIn, Random rand, BlockPos pos)
		{
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int ii = 0; ii < 64; ++ii)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = ChunkGenSpawner.getTopSolidBlock(pos.add(j, 0, k), worldIn).getY() + 1;
					STATIC_GENERATOR.generate(BlockWaterBottomGunk.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 1, 15, 0, 255);
				}
