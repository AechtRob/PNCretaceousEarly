...
public class WorldCretaceousEarly extends ElementsLepidodendronMod.ModElement {
	public static int DIMID = LepidodendronConfig.dimCretaceousEarly;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("lepidodendron:cretaceous_early_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldCretaceousEarly(ElementsLepidodendronMod instance) {
		super(instance, 1590);
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Prehistoric Nature loading error! Dimension ID for the Early Cretaceou dimension is already registered by another mod: " + DIMID);
			return;
		}
		dtype = DimensionType.register("cretaceous_early", "_cretaceous_early", DIMID, WorldProviderMod.class, false);
	...
	public static class WorldProviderMod extends WorldProvider {
		@Override
		public void init() {
			this.biomeProvider = new BiomeProviderCretaceousEarly(this.world.getSeed(), this.world.getWorldInfo());
		...
		public Biome getBiomeForCoordsBody(final BlockPos pos, World worldIn)
		{
			...
			{
				return worldIn.provider.getBiomeProvider().getBiome(pos, BiomeEarlyCretaceousOcean.biome);
			}
		}
		...
		public Biome getBiome(Chunk chunk, BlockPos pos, BiomeProvider provider)
		{
			...
			if (k == 255)
			{
				...
				Biome biome = world.isRemote ? BiomeEarlyCretaceousOcean.biome : provider.getBiome(pos, BiomeEarlyCretaceousOcean.biome);
				k = Biome.getIdForBiome(biome);
				chunk.getBiomeArray()[j << 4 | i] = (byte)(k & 255);
			}

			Biome biome1 = Biome.getBiome(k);
			return biome1 == null ? BiomeEarlyCretaceousOcean.biome : biome1;
		}
		...
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderCretaceousEarly(this.world, this.world.getSeed() - DIMID);
		}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setTranslationKey("pf_cretaceous_early_portal");
			setRegistryName("lepidodendron:cretaceous_early_portal");
			setLightLevel(0F);
		}
			...
			if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
				...
				if (thePlayer.timeUntilPortal > 0) {
					...
					if (thePlayer.dimension == DIMID) {
						...
						ModTriggers.ENTER_CRETACEOUS_EARLY.trigger((EntityPlayerMP)thePlayer);
					}
				} else {
					thePlayer.timeUntilPortal = 10;
					ReflectionHelper.setPrivateValue(EntityPlayerMP.class, thePlayer, true, "invulnerableDimensionChange", "field_184851_cj");
