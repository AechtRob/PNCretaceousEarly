...
public class GenLayerNAmericanCopse extends GenLayer {

    public Biome FIELD = Biome.REGISTRY.getObject(new ResourceLocation("lepidodendron:cretaceous_early_namerica_shrubland"));
    public int FIELD_ID =  Biome.getIdForBiome(FIELD);
    public Biome COPSE = Biome.REGISTRY.getObject(new ResourceLocation("lepidodendron:cretaceous_early_namerica_shrubland_copse"));
    ...
    public GenLayerNAmericanCopse(long seed, GenLayer genlayer) {
        super(seed);
    ...
    private int[] diversify(int x, int z, int width, int height) {
        ...
        for (int zOut = 0; zOut < height; zOut++) {
            for (int xOut = 0; xOut < width; xOut++) {
                ...
                if (Biome.getBiome(center) == BiomeEarlyCretaceousLandNAmericaShrubland.biome) {
