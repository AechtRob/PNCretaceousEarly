...
public class GenLayerAustroAntarcticHighLakes0 extends GenLayer
{
    ...
    public GenLayerAustroAntarcticHighLakes0(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (k == AUSTRO_LAKE_EDGE_ID)
                {
                    ...
                    boolean flag = (
                        (
                        (l1 == AUSTRO_LAKE_EDGE_ID)
                        && (k2 == AUSTRO_LAKE_EDGE_ID)
                        && (j3 == AUSTRO_LAKE_EDGE_ID)
                        && (i4 == AUSTRO_LAKE_EDGE_ID)
                        )
                    );
                    if (flag)
                    {
                        aint1[j + i * areaWidth] = AUSTRO_LAKES_ID;
                    }
                    else {
